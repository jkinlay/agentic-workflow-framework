...
def check_release(root=ROOT, version=None):
    ...
    for path in files:
        ...
        if path.suffix.lower() not in SCAN_SUFFIXES:
            continue
        if 'tests' in rel.parts or path.name in HISTORY or path.name.startswith(('MIGRATION-', 'RED-TEAM-DISPOSITION-')):
            continue
        stale = stale_versions(path.read_text(encoding='utf-8'), current)
        if stale:
