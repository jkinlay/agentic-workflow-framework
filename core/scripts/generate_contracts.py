...
GATE_NAMES = ["acceptance_criteria", "scope", "critic_current_tuple", "specialist_reviews",
              ...
              "merge_compatibility", "ticket_snapshot_current", "review_coverage", "provenance"]
...
def catalog():
    ...
    schemas["worker-result"] = bound({"status": enum("COMPLETE", "BLOCKED", "FAILED"), "dispatch_id": UUID,
        ...
        "validation": arr(obj({"command": text(), "started_at": TIME, "finished_at": TIME, "exit_code": integer(-2147483648),
            "tested_tree_sha": SHA, "clean_checkout": BOOL, "evidence": EVIDENCE}), 1),
        "self_review_complete": BOOL, "findings_addressed": STRINGS, "blockers": STRINGS})
    ...
    review = {"verdict": enum("APPROVE", "REQUEST_CHANGES", "INCOMPLETE"), "acceptance_criteria": arr(AC_RESULT, 1),
              "findings": arr(FINDING), "prior_finding_ids": STRINGS,
    ...
    schemas["blocker"] = bound({"reason_code": text(), "summary": text(), "evidence": EVIDENCE,
        "resume_state": enum("READY", "IN_PROGRESS", "READY_FOR_CRITIC", "CHANGES_REQUESTED", "SPECIALIST_REVIEW", "FINAL_REVIEW", "MERGED_PENDING_JIRA"),
    ...
    schemas["jira-transition"] = bound({"operation_id": UUID, "merge_result_id": UUID,
    ...
    schemas["controller-event"] = obj({"schema_version": const(3), "event_id": UUID, "project_id": UUID,
        "sequence": integer(1), "timestamp": TIME, "actor": enum(*ROLES), "event_type": text(),
    ...
    schemas["evidence-bundle"] = obj({"schema_version": const(3), "candidate": ref("candidate"),
        ...
        "prior_findings": arr(FINDING), "evidence_registry": arr(obj({"uri": text(format="uri"), "sha256": DIGEST,
    ...
    schemas["project-config"] = obj({"version": const(3), "template": obj({"expected_workflow_version": const(VERSION)}),
        ...
        "validation": obj({"commands": arr(text(), 1, uniqueItems=True), "ci_candidate_policy": const("synthetic_merge_required"),
          "require_candidate_bound_ci_evidence": TRUE, "required_ci_checks": arr(obj({"name": text(), "app_id": integer(1),
            "workflow_path": text(), "workflow_sha": SHA, "min_tests_executed": integer(1)}), 1),
        ...
        "merge_gate": obj({"human_authorization_required": TRUE, "execution_after_authorization": const("owner_manual"),
          ...
          "authorization_ttl_seconds": integer(1, maximum=86400), "trusted_owner_ids": arr(integer(1), 1, uniqueItems=True),
          "high_risk_owner_quorum": integer(1)}),
        "controller": obj({k: FALSE for k in ["dispatch_enabled", "auto_dispatch", "auto_request_critic", "auto_resume_amendments", "auto_transition_jira"]}),
    ...
    # Optional for legacy projects. This is a project-wide native reviewer pool,
    # not a per-ticket review quorum or additional observed host capacity.
    schemas["project-config"]["properties"]["execution"]["properties"]["independent_reviewers"] = obj({
        "count": integer(0), "allocation": const("one_per_stream")})
