...
Native dispatch is host-dependent. Default capacity is three workstreams and three independent reviewers total, one per stream. Respect shared host capacity and count only confirmed launches. Keep one writer per path and separate reviewer contexts.

Verify candidate-bound tests, findings and human merge authorization. Report observed outcomes and constraints; continue independent authorized work while a decision is pending. Skill installation does not migrate every project. Never rehash corrupted bytes, reset accounting or invent background execution.
