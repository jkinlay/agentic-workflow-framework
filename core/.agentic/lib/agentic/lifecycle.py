...
REVIEW = {"READY_FOR_CRITIC", "CHANGES_REQUESTED", "SPECIALIST_REVIEW", "FINAL_REVIEW", "READY_FOR_OWNER_AUTHORIZATION", "OWNER_AUTHORIZED"}
AFTER_MERGE = {"MERGED", "MERGED_PENDING_JIRA", "DONE"}
RESUME = {"READY", "IN_PROGRESS", "READY_FOR_CRITIC", "CHANGES_REQUESTED", "SPECIALIST_REVIEW", "FINAL_REVIEW", "MERGED_PENDING_JIRA"}
NORMAL = [
    ...
    ("CHANGES_REQUESTED", "AMENDMENT_ACCEPTED", "AMENDING", ["dispatch_permitted", "lease_current", "budget_reserved", "ownership_current"]),
    ...
    ("MERGED", "JIRA_RECONCILED", "DONE", ["merge_confirmed", "jira_done_confirmed", "dependencies_refreshed"]),
    ("MERGED_PENDING_JIRA", "JIRA_RECONCILED", "DONE", ["merge_confirmed", "jira_done_confirmed", "dependencies_refreshed"]),
...
def transition(state, event, facts=None, resume_state=None):
    ...
    if event == "BLOCK" and state not in {"MERGED", "MERGED_PENDING_JIRA"}:
        require("blocker_recorded")
        return "BLOCKED"
    ...
    if state == "BLOCKED" and event == "RECOVER":
        ...
        if resume_state == "MERGED_PENDING_JIRA":
