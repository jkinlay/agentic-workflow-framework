...
def select_route(policy, request, capabilities):
    ...
    role = request["role"]
    high_risk = request["risk"] == "high" or bool(set(request["risk_flags"]) & set(policy["high_risk_flags"]))
    demanding = high_risk or request["complexity"] == "high" or request["uncertainty"] == "high"
    simple = all(request[x] == "low" for x in ("risk", "complexity", "uncertainty")) and request["verification"] == "strong" and not request["risk_flags"]
    selected = deepcopy(policy["role_defaults"][role])
    reasons = ["balanced role default"]
    if role == "worker" and simple:
        selected = deepcopy(policy["simple_worker"])
    ...
    for field, key in (("agent_overrides", request["agent_id"]), ("ticket_overrides", request["ticket_id"])):
        ...
        if override is not None:
            if floor and not _at_least(policy, override, floor):
                return {"status": "blocked", "reason": "override violates risk/review floor", "policy_sha256": _fingerprint(policy)}
    ...
    if failure_kind in REASONING_FAILURES:
        ...
        escalated = True
        reasons.append("automatic escalation after " + failure_kind + " failure")
...
class RoutingLedger:
    ...
    def reserve(self, project_id, policy, request, capabilities):
        """Reserve hard upper bounds before a new run; does not execute a model."""
        ...
        with self._transaction() as connection:
            ...
            route = select_route(policy, enriched, capabilities)
            ...
            connection.execute("""INSERT INTO model_runs
                               (run_id,project_id,ticket_id,role,agent_id,phase,created_at,day,policy_hash,status,
                                reserved_tokens,reserved_cost,actual_tokens,actual_cost,escalated,request,route,outcome)
                               VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)""",
                               (run_id, project_id, request["ticket_id"], request["role"], request["agent_id"],
                                ...
                                None, None, int(route["escalated"]), json.dumps(enriched), json.dumps(route), None))
    ...
    def settle(self, run_id, outcome):
        ...
        with self._transaction() as connection:
            ...
            recorded = {**outcome, "violations": violations, "settled_at": datetime.now(timezone.utc).isoformat()}
        ...
        with self._transaction() as connection:
            ...
            if row["status"] == "reserved":
                ...
                closed = {"success": False, "failure_kind": "unknown", "reconciled": True,
                          "settled_at": now, "actual_usage_known": False}
    ...
    def evidence(self, project_id, policy):
        ...
        groups = {}
        with closing(self._connect()) as connection:
            rows = connection.execute("SELECT * FROM model_runs WHERE project_id=? AND policy_hash=? AND status IN ('settled','quarantined','reconciled') ORDER BY rowid", (project_id, _fingerprint(policy))).fetchall()
        ...
        for row in rows:
            ...
            key = (request["task_class"], request["role"], route["model"], route["reasoning_effort"], route["simple"], route["high_risk"])
