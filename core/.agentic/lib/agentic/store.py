...
from .lifecycle import STATES, transition
...
class Store:
    ...
    def acquire_lease(self, task_id, owner_id, workers, heavy, gpu, now, ttl_seconds, limits):
        uuid.UUID(task_id)
        ...
        with self.transaction() as db:
            ...
            if old:
                if (old["owner_id"], old["workers"], old["heavy"], old["gpu"]) != (owner_id, workers, heavy, gpu):
    ...
    def reserve_budget(self, run_id, ticket, now, tokens, cost, limits):
        uuid.UUID(run_id)
        ...
        with self.transaction() as db:
            ...
            if old:
                if (old["ticket"], old["tokens"], old["cost"]) != (ticket, tokens, cost):
            ...
            db.execute("INSERT INTO reservations VALUES(?,?,?,?,?,'RESERVED',NULL,NULL)", (run_id, ticket, day, tokens, cost))
            self._event(db, "BUDGET_RESERVED", {"run_id": run_id, "ticket": ticket, "tokens": tokens, "cost": cost}, now=now)
            return "RESERVED"
