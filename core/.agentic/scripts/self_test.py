...
def main(argv=None):
    ...
    try:
        ...
        if len(contracts.schemas) != 29:
            raise ValueError('Expected complete catalog of 29 schemas')
        ...
        forms = list((ROOT / '.agentic/templates').glob('*.yaml')) + list((ROOT / '.agentic/templates').glob('*.json'))
        covered = set()
        for path in forms:
            value = load(path)
        ...
        if covered != set(contracts.schemas) - {'candidate','project-config','evidence-bundle'}:
