...
class NativeCapacityTests(unittest.TestCase):
    ...
    def test_above_three_project_capacity_discloses_structural_planner_limit(self):
        ...
        self.assertEqual(capacity["planner_writer_limit"], 3)
        self.assertEqual(capacity["effective_writer_capacity"], 3)
        self.assertEqual(capacity["limiting_sources"], ["planner_structure"])
        self.assertEqual(len(plan["dispatch_packets"]), 3)
        self.assertEqual(self.execution, before)
        self.assertIn("remains configured as 5", " ".join(capacity["reasons"]))
        self.assertIn("Planner structural writer limit: 3", render_markdown(plan).decode())
    ...
    def test_binding_capacity_sources_distinguish_lower_project_host_and_depth(self):
        for project, host, depth, sources in [(2, 5, 0, ["project_configuration"]),
                                              (5, 1, 0, ["observed_host_capacity"]),
                                              (5, 5, 1, ["spawn_depth"]),
                                              (2, 2, 0, ["observed_host_capacity", "project_configuration"])]:
    ...
    def test_lower_project_limit_defers_ready_work_without_changing_the_three_stream_plan(self):
    ...
    def test_malformed_execution_capacity_and_depth_inputs_are_rejected(self):
        for field, value in [("max_parallel_tickets", True), ("max_parallel_tickets", 0),
                             ...
                             ("max_spawn_depth", 2), ("max_parallel_tickets_per_stream", 4),
...
class PlannerCliTests(unittest.TestCase):
    ...
    def test_cli_applies_configured_limit_and_records_observed_host_capacity(self):
        with tempfile.TemporaryDirectory(prefix="awf-cap-") as temporary:
            ...
            project.mkdir()
            configuration = {"execution": {**deepcopy(DEFAULT_NATIVE_EXECUTION), "max_parallel_tickets": 2},
                             "controller": {"dispatch_enabled": False, "auto_dispatch": False},
                             "host_broker": {"enabled": False, "max_workers": 1}}
            ...
            self.assertEqual(len(report["deferred_dispatch_packets"]), 1)
