...
def run_validation(source, expected_manifest_sha256, workdir, python_executable=None):
    ...
    try:
        ...
        # Complete explicit synthetic metadata prevents live discovery. The test
        ...
        bootstrap_report = _json(bootstrap.stdout)
        require(bootstrap_report["status"] == "CONFIGURED" and bootstrap_report["installed"] is True
                and bootstrap_report["active"] is False, "Synthetic Git fixture was not configured without activation")
        require(bootstrap_report["source_manifest_sha256"] == expected_manifest_sha256,
        ...
        integrity, accepted = (item["output"] for item in observed)
        require(integrity.get("integrity_valid") is True
                and integrity.get("source_manifest_sha256") == expected_manifest_sha256,
                "Installed integrity check did not verify the approved manifest")
        pre_install = bootstrap_report["pre_install_configuration"]
        configuration_report = bootstrap_report["configuration"]
        policy_digest = accepted.get("policy_sha256")
        require(accepted.get("status") == "ACCEPTED" and accepted.get("unresolved") == []
                and isinstance(policy_digest, str) and re.fullmatch(r"[0-9a-f]{64}", policy_digest)
                and pre_install.get("status") == "ACCEPTED" and pre_install.get("policy_sha256") == policy_digest
                and configuration_report.get("status") == "ACCEPTED" and configuration_report.get("unresolved") == []
                and configuration_report.get("policy_sha256") == policy_digest
                and configuration_report.get("observation_source") == "installed validate-config subprocess",
                "Installed configuration did not match the inspected bootstrap policy")
        discovery = pre_install["discovery"]
        check("Synthetic Git fixture proves both installed checks without metadata lookup or project test execution",
              discovery["observations"] == [] and discovery["test_commands_executed"] is False,
              source_manifest_sha256=expected_manifest_sha256, policy_sha256=policy_digest)
        report["installed_bootstrap"] = bootstrap_report
