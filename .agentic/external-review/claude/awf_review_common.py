...
MODEL_PROCESS_CLEANUP_SECONDS = 2
MAX_MODEL_OUTPUT_TOKENS = 8000
...
def strict_json(raw: bytes | str, label: str) -> Any:
    def pairs(items):
        ...
        for key, value in items:
            if key in result:
                raise AdapterError(f"{label} has duplicate JSON keys")
            result[key] = value
        return result
    def constant(_value):
        raise AdapterError(f"{label} has a non-finite JSON number")
    try:
        text = raw.decode("utf-8") if isinstance(raw, bytes) else raw
        return json.loads(text, object_pairs_hook=pairs, parse_constant=constant)
    except (UnicodeError, ValueError, RecursionError) as err:
        raise AdapterError(f"{label} is not valid strict UTF-8 JSON") from err
...
def json_bytes(value: Any) -> bytes:
    try:
        ...
        raise AdapterError("value cannot be serialized as finite UTF-8 JSON") from err


def bounded_utf8(text: str, limit: int) -> tuple[str, bool]:
    if not isinstance(limit, int) or isinstance(limit, bool) or limit < 1:
        raise AdapterError("byte limits must be positive integers")
    raw = text.encode("utf-8")
    return (text, False) if len(raw) <= limit else (raw[:limit].decode("utf-8", errors="ignore"), True)
...
def read_text_file(path: str, limit: int) -> tuple[str | None, bool, bool]:
    ...
    text, text_truncated = bounded_utf8(raw[:limit].decode("utf-8", errors="replace"), limit)
    return text, len(raw) > limit or text_truncated, False
