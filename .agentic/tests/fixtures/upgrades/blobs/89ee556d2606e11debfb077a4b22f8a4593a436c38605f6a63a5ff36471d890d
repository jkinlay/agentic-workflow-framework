...
def find_upwards(relative: str) -> str:
    """Resolve the bundled component, never a surrounding project's files."""
    component = os.path.dirname(ADAPTER)
    if relative.startswith(".github/workflows/"):
        return os.path.join(component, "workflows", os.path.basename(relative))
    if relative.endswith("review-report.schema.json"):
        return os.path.join(component, "review-report.schema.json")
    here = ADAPTER
    for _ in range(8):
...
POLICY_TEXT = textwrap.dedent(f"""\
    ...
    review:
      ...
      qualification:
        ...
        environment_secret_boundaries_verified: true
...
class RunModelTests(unittest.TestCase):
    ...
    def test_active_workflow_sets_opus5_runtime_bounds(self):
        ...
        self.assertRegex(model_job, r"(?m)^          --max-tokens 8000$")
        self.assertRegex(model_job, r"(?m)^          --timeout 300$")

    def test_workflow_is_opt_in_and_separates_model_from_publisher(self):
        with open(find_upwards(".github/workflows/awf-review-claude.yml"), encoding="utf-8") as handle:
            workflow = handle.read()
        model_job, publisher_job = workflow.split("  publish:", 1)
        self.assertIn("vars.AWF_ENABLE_CLAUDE_REVIEW == 'true'", model_job)
        self.assertIn("github.event.pull_request.head.repo.full_name == github.repository", model_job)
        self.assertIn("pull-requests: read", model_job)
        self.assertNotIn("pull-requests: write", model_job)
        self.assertNotIn("AWF_REVIEWER_APP_PRIVATE_KEY", model_job)
        self.assertNotIn("create-github-app-token@", model_job)
        self.assertNotIn("ANTHROPIC_API_KEY", publisher_job)
        self.assertNotIn("run_model.py", publisher_job)
        self.assertIn("needs.model.result == 'success'", publisher_job)
        self.assertIn('--expected-head "${{ needs.model.outputs.head_sha }}"', publisher_job)
        self.assertEqual(workflow.count("persist-credentials: false"), 3)
        self.assertNotRegex(workflow, r"(?m)^\s*working-directory:\s*pr")
...
class VerifyRecordTests(unittest.TestCase):
    def test_failed_or_duplicate_fragments_cannot_suggest_qualification(self):
        required = ('model-negative', 'publisher-negative', 'app-permissions', 'unit', 'hashes')
        for failed in ('unit', 'hashes', 'duplicate'):
            with self.subTest(failed=failed), tempfile.TemporaryDirectory() as tmp:
                paths = []
                for name in required:
                    path = os.path.join(tmp, name + '.json')
                    with open(path, 'w', encoding='utf-8') as handle:
                        json.dump({'step': name, 'pass': name != failed}, handle)
                    paths.append(path)
                if failed == 'duplicate':
                    paths.append(paths[0])
                with redirect_stdout(io.StringIO()):
                    code = verify.main(['record', '--inputs', *paths, '--out', os.path.join(tmp, 'record.json')])
                self.assertEqual(code, 1)

    def test_hash_fragment_covers_policy_and_both_workflows(self):
        with tempfile.TemporaryDirectory() as tmp, redirect_stdout(io.StringIO()):
            path = os.path.join(tmp, 'hashes.json')
            self.assertEqual(verify.main(['hashes', '--schema', SCHEMA_PATH, '--out', path]), 0)
            with open(path, encoding='utf-8') as handle:
                hashes = json.load(handle)['sha256']
        self.assertEqual(len(hashes), 10)
        self.assertTrue(any(path.endswith("model_transport.py") for path in hashes))
        for name in ('policy.py', 'workflows/awf-review-claude.yml', 'workflows/awf-review-gate.yml'):
            self.assertRegex(hashes[name], '^[0-9a-f]{64}$')

    def test_live_probes_require_exact_confirmation_before_any_http(self):
        for command in ("model-negative", "publisher-negative"):
            for confirmation in (None, "o/other", "O/r", "o/r/", " o/r", "o/r "):
                with self.subTest(command=command, confirmation=confirmation):
                    argv = [command, "--repo", "o/r", "--pr", "7", "--head-sha", SHA_A]
                    if confirmation is not None:
                        argv += ["--disposable-repository-confirmation", confirmation]
                    with mock.patch.object(verify, "http_json") as http, \
                            mock.patch.object(verify, "resolve_head") as resolve, \
                            mock.patch.object(verify, "write_fragment") as write, \
                            mock.patch.dict(os.environ, {"GITHUB_TOKEN": "test", "GH_TOKEN": "test"}), \
                            redirect_stderr(io.StringIO()) as err:
                        code = verify.main(argv)
                    self.assertEqual(code, 2)
                    self.assertIn("disposable-repository-confirmation OWNER/REPO", err.getvalue())
                    http.assert_not_called()
                    resolve.assert_not_called()
                    write.assert_not_called()

    def test_exact_confirmation_allows_only_mocked_probe_targets(self):
        """Every API operation here is fake; this test never qualifies a live host."""
        for command in ("model-negative", "publisher-negative"):
            calls = []

            def fake_http(method, url, token=None, body=None):
                self.assertTrue(url.startswith(common.GITHUB_API_DEFAULT + "/repos/o/r/") or
                                url == common.GITHUB_API_DEFAULT + "/repos/o/r")
                calls.append((method, url, body))
                if method == "GET" and url.endswith("/repos/o/r"):
                    return 200, {"id": 123, "full_name": "o/r"}, {}
                if method == "GET" and url.endswith("/pulls/7"):
                    return 200, {"head": {"sha": SHA_A, "repo": {"id": 123}}, "base": {"repo": {"id": 123}}}, {}
                return (200 if method == "GET" else 403), {}, {}

            with self.subTest(command=command), tempfile.TemporaryDirectory() as tmp, \
                    mock.patch.object(verify, "http_json", fake_http), \
                    mock.patch.dict(os.environ, {"GITHUB_TOKEN": "test", "GH_TOKEN": "test",
                                                 "GITHUB_API_URL": common.GITHUB_API_DEFAULT}), \
                    redirect_stdout(io.StringIO()):
                out = os.path.join(tmp, "fragment.json")
                marker = os.path.join(tmp, "canary-marker.json")
                with open(marker, "w", encoding="utf-8") as handle:
                    json.dump({"format": "awf-disposable-canary-1", "repository": "o/r", "repository_id": 123,
                               "pr_number": 7, "head_sha": SHA_A, "expires_at": int(verify.time.time()) + 3600,
                               "disposable": True, "allowed_probes": [command]}, handle)
                code = verify.main([command, "--repo", "o/r", "--pr", "7", "--head-sha", SHA_A,
                                    "--disposable-repository-confirmation", "o/r", "--out", out,
                                    "--canary-marker", marker, "--canary-marker-sha256", common.sha256_file(marker)])
                with open(out, encoding="utf-8") as handle:
                    fragment = json.load(handle)
            self.assertEqual(code, 0)
            self.assertTrue(fragment["pass"])
            self.assertEqual(fragment["step"], command)
            self.assertTrue(any(method != "GET" for method, _, _ in calls))

    def test_negative_probe_cli_rejects_missing_confirmation(self):
        import subprocess
        for command in ("model-negative", "publisher-negative"):
            env = dict(os.environ, GITHUB_TOKEN="", GH_TOKEN="")
            result = subprocess.run([sys.executable, "-B", os.path.join(ADAPTER, "verify.py"),
                                     command, "--repo", "o/r", "--pr", "7"],
                                    capture_output=True, text=True, env=env, timeout=15)
            self.assertEqual(result.returncode, 2)
            self.assertIn("disposable-repository-confirmation OWNER/REPO", result.stderr)
    ...
    def test_record_requires_every_permission_step(self):
        ...
        self.assertFalse(record["suggested_qualification"]["environment_secret_boundaries_verified"])
