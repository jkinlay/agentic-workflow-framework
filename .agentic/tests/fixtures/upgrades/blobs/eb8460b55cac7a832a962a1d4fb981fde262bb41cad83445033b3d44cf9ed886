...
def parse_args(argv: list[str]) -> argparse.Namespace:
    ...
    parser.add_argument("--max-input-bytes", type=int, default=MAX_INPUT_BYTES,
                        help="aggregate serialized UTF-8 input cap; may lower but not raise the hard limit")
...
def build(args: argparse.Namespace, token: str) -> dict:
    for name in ("max_diff_bytes", "max_file_bytes", "max_files", "max_contract_bytes", "max_report_bytes", "max_input_bytes"):
        value = getattr(args, name)
        if not isinstance(value, int) or isinstance(value, bool) or value < 1:
            raise AdapterError(f"{name} must be a positive integer")
    if args.max_input_bytes > MAX_INPUT_BYTES:
        raise AdapterError(f"max_input_bytes exceeds the hard {MAX_INPUT_BYTES}-byte cap")
    ...
    diff_text, diff_truncated = bounded_utf8("\n".join(diff_parts), args.max_diff_bytes)
    if diff_truncated:
        limitations.append(f"The unified diff was truncated at {args.max_diff_bytes} UTF-8 bytes.")
    ...
    def check_budget() -> None:
        # Account for actual JSON escaping and metadata before reading another
        # whole file; the exact same serialization is written by main().
        if len(json_bytes(payload)) > args.max_input_bytes:
            raise AdapterError(f"aggregate serialized UTF-8 review input exceeds {args.max_input_bytes} bytes; reduce the reviewed change or explicit per-file bounds")
    ...
    for entry in changed:
        ...
        if target is None or not os.path.isfile(target) or os.path.islink(target):
            ...
            continue
        text, truncated, binary = read_text_file(target, min(args.max_file_bytes, args.max_input_bytes + 1))
        if binary:
    ...
    if contract_path and os.path.isfile(contract_path):
        text, truncated, binary = read_text_file(contract_path, min(args.max_contract_bytes, args.max_input_bytes + 1))
        if not binary:
            task_contract = text
            if truncated:
    ...
    if report_path and os.path.isfile(report_path):
        text, truncated, binary = read_text_file(report_path, min(args.max_report_bytes, args.max_input_bytes + 1))
        if not binary:
    ...
    payload["execution_report"] = execution_report
    check_budget()
    return payload
