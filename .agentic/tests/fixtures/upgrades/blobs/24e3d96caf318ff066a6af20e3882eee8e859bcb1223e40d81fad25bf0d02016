...
DEFAULT_NATIVE_EXECUTION = {
    ...
    "max_parallel_tickets": 3, "max_parallel_tickets_per_stream": 1, "max_spawn_depth": 1,
}
...
def inventory(raw, expected_sha256, now, allow_synthetic=False):
    ...
    for epic in value["epics"]:
        fields(epic, "id title scope", "Epic")
        key(epic["id"], "Epic ID")
    ...
    for ticket in value["tickets"]:
        ...
        for path in ticket["write_paths"]:
            ...
            require(path.split("/")[0].casefold() not in {".git", ".agentic", ".codex"}
                    and path.split("/")[-1].casefold() not in {"agents.md", "codeowners"}
                    and not overlap(path, ".github/workflows")
                    and path.casefold() not in {PLAN.casefold(), MARKDOWN.casefold(), LOCK, JOURNAL},
...
def native_execution_policy(execution=None):
    """Validate the native subset; reference-controller/broker flags are separate."""
    ...
    fields(reviewers, "count allocation", "execution.independent_reviewers")
    ...
    return {name: deepcopy(reviewers if name == "independent_reviewers" else value[name]) for name in DEFAULT_NATIVE_EXECUTION}
...
def _reviewer_plan(streams, packets, capacity, policy):
    ...
    count = policy["independent_reviewers"]["count"]
    assignments = []
...
def _select_capacity(streams, packets, policy, host_writer_capacity, coordinator_spawn_depth,
    ...
    physical_ceiling = min(PLANNER_WRITER_LIMIT, policy["max_parallel_tickets"]) if enabled else 0
    physical_limit = min(physical_ceiling, host_writer_capacity) if host_writer_capacity is not None else physical_ceiling
    ceiling = min(physical_ceiling, depth_limit)
    ...
    available = max(0, planning_limit - retained_count)
    ...
    if policy["max_parallel_tickets"] > PLANNER_WRITER_LIMIT:
        reasons.append(f"The A/B/C planner has a structural limit of {PLANNER_WRITER_LIMIT} writers; execution.max_parallel_tickets remains configured as {policy['max_parallel_tickets']}. Supporting additional streams requires a planner implementation change, not an autonomous increase to project limits.")
    if policy["max_parallel_tickets"] < PLANNER_WRITER_LIMIT:
    ...
    if policy["max_parallel_tickets_per_stream"] != 1:
        reasons.append("The A/B/C planner keeps one writer per stream even when the broader configuration permits more.")
    if not can_spawn:
...
def plan_inventory(raw, expected_sha256, now, allow_synthetic=False, previous=None, *,
    ...
    independent = [sum(1 << right for right in range(len(potential)) if right != left
                       and not (reaches[left] & (1 << right)) and not (reaches[right] & (1 << left)))
                   for left in range(len(potential))]
    anchors = potential[:1]
    found = False
    for left in range(len(potential)):
        for right in range(left + 1, len(potential)):
            if not independent[left] & (1 << right):
                continue
            anchors = [potential[left], potential[right]]
            common = independent[left] & independent[right] & ~((1 << (right + 1)) - 1)
            if common:
                third = (common & -common).bit_length() - 1
                anchors.append(potential[third])
                found = True
                break
        if found:
            break
    ...
    ready_groups = [group for group, members in groups.items() if any(_ready(tickets[t])
                    and not (tickets[t]["ownership"] and tickets[t]["ownership"]["state"] == "paused") for t in members)]
    count = max(1 if groups else 0, len(anchors), len(existing_labels))
    labels = sorted(existing_labels)
    for label in LABELS:
        if len(labels) < count and label not in labels:
            labels.append(label)
    labels.sort()
...
def render_markdown(plan):
    ...
    lines.extend(["## Effective native writer capacity", "",
                  ...
                  f"Planner structural writer limit: {capacity['planner_writer_limit']} (streams A/B/C). Binding capacity source(s): {', '.join(capacity['limiting_sources'])}.", "",
