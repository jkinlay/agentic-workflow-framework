...
Upgrades preserve the existing `PROJECT_CONFIG.yaml` bytes, including 1.9.1 monetary and token limits. To opt in, an owner reviews a governance PR that changes both execution monetary limits to `null` and adjusts token limits deliberately. Do not silently rewrite accepted project budgets.
