---
name: awf
description: Verify and adopt AWF releases; guide project work with bounded model routing, independent review and evidence accounting. Includes AWF 1.8.
metadata:
...
Verify and prepare [bundled source](assets/agentic-workflow-template-v1.8.zip) against [pins](assets/release.json):

```text
python -B PATH_TO_SKILL/scripts/prepare_release.py --cache-dir ABSOLUTE_EXTERNAL_CACHE --version 1.8
```
...
Escalation within approved limits needs no repeated prompt. Cap increases, expanded model allowlists, weaker reviews and reconciliation enablement require explicit human direction. Candidate policy cannot approve itself. Reconciliation records operator assertions, not authenticated attestations; the host must verify termination and authority. Preserve charges, history and the finite ticket-wide retry limit.
