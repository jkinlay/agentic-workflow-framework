...
A fresh installation into an empty skills root needs no earlier AWF skill. When upgrading, do not uninstall first. The helper verifies exact inventory, stages replacement, preserves catalog/update-channel/local settings and retains the complete old folder outside scanned skills directories. Custom `--preserve-relative` selections persist in receipts. Identical reruns verify without replacement; unknown/conflicting/newer versions fail closed.
