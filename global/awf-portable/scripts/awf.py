...
def locate(catalog_path, requested_version=None):
    ...
    try:
        if requested_version is not None and (not isinstance(requested_version, str) or
            ...
            fail("INVALID_VERSION_SELECTOR", "Use major.minor or exact major.minor.patch, for example 1.5")
...
def managed(path):
    return path in {"AGENTS.md", ".github/PULL_REQUEST_TEMPLATE.md", ".github/CODEOWNERS"} or path.startswith(".agentic/")
        ...
        try:
            ...
            if receipt["template_version"] != proof["version"] or receipt.get("source_manifest_sha256") != proof["manifest_sha256"]:
                ...
                expected = {name: sha for name, sha in manifest["files"].items()
                            if managed(name) and name not in {CONFIG, VERSION_FILE, ".github/CODEOWNERS"}}
